...
[source,cpp,indent=0]
----
include::src/python.cc[tag=native]
----
...
Using Cython simplifies the embedding as declarations are in Cython files
(with all Python features available) and C/C++ side requires only registering the module.
Please note that we don't use `--embed` feature (that generates `main`) as we need
to extract Python libraries first. Cython's `main` requires Python modules already available,
so the special environments require custom `main`.

.Declare external functions in `.pyx`-files:
[source,cython,indent=0]
----
include::src/cython_main.pyx[tag=declaration]
----

.Register Cython module
[source,cpp,indent=0]
----
include::src/python.cc[tag=cython]
----
